...
async def analysis_agent(state: AgentState) -> AgentState:
    logger.info("---Entering analysis_agent---")
    try:
        ...
        if not agent_plan or not agent_plan.get("tools"):
            logger.warning("No tools specified for analysis_agent in plan. Returning state unchanged.")
        ...
        # LLM to synthesize final answer
        llm = ChatGroq(
            temperature=0.2,
        ...
        if fallback:
            prompt = ChatPromptTemplate.from_messages([
                ("system", """
                You are an expert analyst. The current date and time is {current_time_date_str}
                ...
                Chat History:
                {chat_history}
                Important: Do not mention that you are using tool outputs or document retrieval. Just answer naturally.
                """),
        ...
        logger.info(f"Analysis agent executed tools and synthesized final response.")
        logger.info("---End of analysis_agent---")
        state.processing.executed_steps.append("analysis_agent")
        return state
    except Exception as e:
        logger.error(f"Error in analysis agent: {str(e)}")
        state.error.error = f"Analysis error: {str(e)}"
        state.response.response = "Error in analysis agent."
        state.processing.current_agent = "analysis_agent"
        logger.info(f"Returning from analysis_agent with type: {type(state)} (error path)")
        logger.info("---End of analysis_agent---")
        return state